...
class HTTPIndex(Index):
    ...
    def generate(self, docnames=None):
        ...
        items = ((method, path, info)
            for method, routes in list(self.domain.routes.items())
        ...
        content = sorted(list(content.items()), key=lambda k: k[0])
        return (content, True)
...
class HTTPDomain(Domain):
    ...
    def clear_doc(self, docname):
        for typ, routes in list(self.routes.items()):
            for path, info in list(routes.items()):
                if info[0] == docname:
                    del routes[path]
    ...
    def get_objects(self):
        for method, routes in list(self.routes.items()):
            for path, info in list(routes.items()):
